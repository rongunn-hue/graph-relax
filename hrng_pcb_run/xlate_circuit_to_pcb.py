...
def parse_circuit(path):
    ...
    (circuit title, placement hints, source_metadata) is real data for OTHER purposes (schematic placement,
    provenance) but irrelevant to what a PCB needs, so it's simply skipped here, not misparsed."""
    ...
    for raw in open(path).read().splitlines():
        ...
        if kw == "component" and len(parts) == 3:
            ...
            nets.append({"name": parts[1], "terminals": parts[2:]})
...
def build_board(components, nets, nc, overrides, libdir, out_path, ncols):
...
def main():
    ...
    unmapped_pads = build_board(components, nets, nc, overrides, args.libdir, args.output, args.cols)
